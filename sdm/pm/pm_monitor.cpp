#include <stdio.h>
#include <unistd.h>
#include <sys/types.h>
#include <signal.h>
#include <stdlib.h>
#include <arpa/inet.h>
#include <netinet/in.h>
#include <sys/socket.h>
#include <sys/wait.h>
#include <string.h>
#include "../common/message/SDMReady.h"
#include "../common/MessageManager/MessageManager.h"
#include "../common/TCPcom.h"
#include "../common/message/SDMHeartbeat.h"
#include "../common/Time/SDMTime.h"

#define NUM_HEARTBEAT_TRIES	10	/*Number of times to miss a response before restarting the PM*/

int StartPM(char **);
void SigIntHandler(int);

int pm_pid = -1;		//Process id number of the Process Manager

int main(int argc, char ** argv)
{
	int result = -1;
	int miss_count = 0;
	int num_toget = 0;
	char buf[BUFSIZE];
	SDMHeartbeat heartbeat;
	MessageManager mm;
	SDMComponent_ID pm;
	
	if (argc == 1)
	{
		printf("Usage:\n%s pm_process [pm_process_args...]\n", argv[0]);
		return -1;
	}
	// Set up the command line for the child process
	char **nargv = new char*[argc];
	for (int i = 1;  i < argc;  ++i)
		nargv[i-1] = argv[i];
	nargv[argc-1] = NULL;
	
	//Set monitor process address
	heartbeat.source.setSensorID(0);
	heartbeat.source.setAddress(inet_addr("127.0.0.1"));
	heartbeat.source.setPort(PORT_PM_MONITOR);
	
	//Set PM address
	pm.setAddress(inet_addr("127.0.0.1"));
	pm.setPort(PORT_PM);
	pm.setSensorID(0);
	
	signal(SIGINT, SigIntHandler);
	pm_pid = StartPM(nargv);
	mm.Async_Init(PORT_PM_MONITOR);
	
	//If fork/exec was successful
	if (pm_pid > 0)		
	{
		//Allow the PM to get started up
		sleep(HEARTBEAT_INTERVAL);
		while (1)
		{
			
			//Send heartbeats via UDP and TCP
			heartbeat.SendTo(pm);
			sleep(HEARTBEAT_INTERVAL);
			
			//If Process Manager quit
			if (waitpid(-1,&result,WNOHANG) == pm_pid)
			{
				printf("  Monitor:  PM failed, restarting...\n");
				pm_pid = StartPM(nargv);
				if (pm_pid > 0)
				{
					sleep(HEARTBEAT_INTERVAL);
					continue;
				}
				else
				{
					printf("  Monitor:  Could not restart the PM.\n");
					return -1;
				}
			}
			else if (mm.IsReady())
			{
				//Should respond with one message
				num_toget = 1;
				
				while (mm.IsReady())
				{
					num_toget--;
					mm.GetMessage(buf);
				}
				if (num_toget > 0)
					miss_count++;
				else
					miss_count = 0;
			}
			else
			{
				//If no messages were received
				miss_count++;
				if (miss_count == NUM_HEARTBEAT_TRIES)
				{
					printf("  Monitor:  PM unresponsive, restarting...\n");
					
					// Be nice first
					kill(pm_pid, SIGINT);
					usleep(10000);
					
					// Then be mean
					if (kill (pm_pid, SIGKILL) == 0)
						wait(&result);
					else
						waitpid(-1, &result, WNOHANG);
					
					pm_pid = StartPM(nargv);
					if (pm_pid > 0)
					{
						sleep(HEARTBEAT_INTERVAL);
						continue;
					}
					else
					{
						printf("  Monitor:  Could not restart the PM.\n");
						return -1;
					}
					
					
				}
			}
		}
	}
	else
	{
		printf("  Monitor:  Error starting the PM (%d).\n", pm_pid);
		return -1;
	}	
	return 0;
}

int StartPM(char ** argv)
{
	int pid;
	
	pid = vfork();
	//Child Process
	if (pid == 0)
	{
		//Start the Process Manager
		if (execvp(argv[0],argv) < 0)
		{
			printf("  Monitor:  Error exec'ing the PM.\n");
			exit(-1);
		}
		//Here to make the compiler happy, never runs however
		return 0;
	}
	//Parent Process
	else
		return pid;
}

void SigIntHandler(int sig_num)
{
	int result = 0;
	if (kill (SIGINT, pm_pid) == 0)
		wait (&result);
	else
		waitpid(-1,&result,WNOHANG);
	
	exit(EXIT_SUCCESS);
}

